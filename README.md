...
- **Python tools** — 262 CLI scripts (all stdlib-only, zero pip installs)
- **Reference docs** — templates, checklists, and domain-specific knowledge

**One repo, eleven platforms.** Works natively with Claude Code, and converts to 10 other tools via `scripts/convert.sh`. All 262 Python tools run anywhere Python runs.
...
262 CLI tools ship with the skills (all verified, stdlib-only):
...
Yes. All 262 Python CLI tools use the standard library only — zero pip installs required. Every script is verified to run with `--help`.
